...
// scroll" pattern.
//
// rootMargin top is shrunk by the editor header height so a title that
// has scrolled *under* the header (still technically in the window
// viewport) is treated as out-of-view. Without this, the header label
// would only appear once the title fully cleared the header, leaving a
// window where both titles read at once.
...
export function useObservePageTitle(ref: RefObject<HTMLElement | null>): void {
  useEffect(() => {
    ...
    const headerH = parseHeaderHeight() ?? 52
