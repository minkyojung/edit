...
export const createBootstrapSlice = (
  ...
  bootstrap: async () => {
    ...
    try {
      ...
      // link), else fall back to a restored tab, else the first doc in
      // the vault. Null only when the vault is empty — the app shows an
      // empty state until the user creates / opens a note.
      const urlSlug = getActiveSlugFromHash()
      const slugToOpen =
        urlSlug && knownSlugs.has(urlSlug)
          ? urlSlug
          : openSlugs[0] ?? scanned[0]?.slug ?? null
      if (slugToOpen) {
