...
function buildDecos(
  ...
  // Phase F: the pending pass is retired. Staged proposals now have
  // their own inline review widget on the page, which carries the
  // full diff + Keep/Reject buttons — a separate gutter bar would
  // duplicate that signal. The committed pass above continues to
  // surface "where AI just committed" once the user accepts.
