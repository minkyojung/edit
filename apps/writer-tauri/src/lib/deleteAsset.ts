// One delete path for the three agent-asset kinds — skills, commands, agents —
...
// (`skills/<dir>/SKILL.md` + assets), commands / agents are a single `.md`.
...
const COMMAND_RE = /^_system\/agent\/commands\/[^/]+\.md$/
const AGENT_RE = /^_system\/agent\/agents\/[^/]+\.md$/

/** True when `rel` is a skill / command / agent file — i.e. the ⋯ menu should
 * route its Delete through {@link deleteAssetByPath} instead of the plain
 * note-trash path. */
export function isAgentAssetPath(rel: string): boolean {
  return SKILL_RE.test(rel) || COMMAND_RE.test(rel) || AGENT_RE.test(rel)
}

/** Delete a skill / command / agent by its vault path. Tears it out of the
...
export async function deleteAssetByPath(rel: string): Promise<void> {
  ...
  const isAsset = isSkill || COMMAND_RE.test(rel) || AGENT_RE.test(rel)
  if (!isAsset) return
  ...
  // Commands / agents are a single file → OS trash (recoverable).
  if (isSkill) {
