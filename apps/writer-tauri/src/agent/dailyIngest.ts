...
export const DAILY_INGEST_PROMPT = `You are filing durable facts from the user's daily journal note into their wiki. Read the note at the given path. Extract only durable, consequential knowledge — a specific non-obvious fact about an entity (a person, book, project, idea), or a concept / framework / method worth re-finding later. Skip transient signals: today's lunch, fleeting moods, weather, routine logistics.
