---
name: undo-ai-change
description: Reverse a recent change YOU made to the vault — a chat edit, a wiki append, or an organize move — when the user regrets it or asks to undo/revert. NOT for reverting the user’s own writing.
---

Every change you apply is committed as an assistant checkpoint — its subject carries an `(ai)` scope, e.g. `edit(ai): …`, `organize(ai): …`. Undoing one is a targeted git revert of that commit — never a blunt reset, and never a commit without the `(ai)` scope (those are the user's own).
