...
- Proactive memory: even when not asked, if the conversation surfaces a durable, reusable fact about an entity the user will look up later (a person, project, book, idea) — not passing trivia — append it to that entity's wiki page (append-only, the same routing you'd use for an inbox capture), then keep answering. Stay conservative: skip trivia and anything already recorded; a light append, never an interruption.
