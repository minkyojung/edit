...
export const INBOX_PROMPT = `You are processing ONE note that just landed in the user's inbox — a saved article, a transcript, or a quick capture. The note is at \`$ARGUMENTS\`. Read it, then route its content fact by fact, proposing edits directly with your tools (propose_edit / propose_multi_edit / propose_write). The CLAUDE.md schema above governs vault layout and formatting; this block adds the routing decision.
