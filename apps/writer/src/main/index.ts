...
app.whenReady().then(() => {
  ...
  ipcMain.on('chat:send', (_, messages: ChatMessage[], documentContext: string | null, model: string, effort: string) => {
    chat(messages, documentContext, model, effort, win.webContents).catch((err) =>
