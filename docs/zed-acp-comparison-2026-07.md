...
> ---
>
> **정정 이력 (2026-07-29)** — §8의 0~4를 실제로 구현하면서 처방 셋이 틀렸고
> 하나가 불충분했다. 이 항목들은 선행 감사에서 그대로 옮겨온 것이라 **같은 오류가
> 양쪽에 있다**. 근거는 선행 감사의 ⚠️ 정정 블록에 있고, 여기서는 §8 표와
> 이 문서 고유의 오류(§0.3)를 고친다.
>
> | # | 원래 처방 | 판정 |
> |---|---|---|
> | 0 | 테스트 20개 부활 | 18개 |
> | 1 | 토큰·키 캐시 | 키만 |
> | 2 | `restart`에서 `hard_kill` | **`Drop`.** 원안대로면 재시작 폭주 |
> | 3 | 릴레이에 `?? bgTurnRunId` | 불충분 |
> | 4 | `modelBodyBase`를 runId로 | **threadId** |
> | 5 | Zed의 단일 비행 파싱 래치 | **적용 불가** — Zed는 백그라운드 파싱, 우리는 렌더 중 동기 (§3-2) |
> | 6 | 타이머 + 적응형 예산 | 타이머가 아니라 **커밋 속도 제한** |
> | §0.3 / 13 | "chat 경로는 Write를 안 막는다" | **사실 아님 — 이미 막혀 있다** |
>
> §5(Zed 약점 표)와 §8(실행표)이 #5에 대해 서로 반대를 적고 있었고, **§5가 맞았다.**
...
> **⚠️ 정정 (2026-07-29) — 마지막 문장이 사실이 아니다. chat도 이미 하고 있다.**
>
> `chat/index.ts:1002-1013`이 명시적 최소권한 목록이고, `Write`/`Edit`/`MultiEdit`/
> `NotebookEdit`가 **전부 빠져 있다**. 바로 위 주석이 이유까지 적어놨다:
> *"Phase E6: explicit least-privilege builtin set. Write-side built-ins … are
> OMITTED — the LLM uses the host-applies `propose_*` MCP tools."* 그리고
> 사이드카에는 `writeFile` 호출이 **0건**이다(감사 3-7의 "사이드카가 디스크 직접"은
> SDK가 CLI 서브프로세스 안에서 쓴다는 뜻이지 사이드카 코드가 쓴다는 뜻이 아니다).
>
> **남은 구멍은 `Bash` 하나다.** builtinTools에 `Bash`가 있고, 샌드박스는 모델이
> 진짜 `git revert`를 돌릴 수 있도록 `.enc` 3개만 막게 좁혀둔 상태라 셸로 볼트에
> 쓸 수 있다.
>
> **따라서 §8 항목 13의 우선순위는 내려가야 한다.** "쓰기 주인이 셋"이라는 프레이밍은
> 과장이고, 실제로 남은 건 (a) `Bash` 구멍 (b) Zed식 diff-against-snapshot이다.
> 그런데 (b)는 CAS 재시도 루프를 없애는 **UX 개선**이지 유실 수정이 아니다 —
> 유실의 실제 원인은 감사 3-1(기준선 키잉)이었고 그건 `f7a47993c`로 고쳤다.
...
### 3-2. 단일 비행 파싱 래치 — 3줄이 순진한 전체 재파싱을 살린다 ← ⚠️ **Octave엔 적용 불가** (절 끝 정정)
...
> **⚠️ 정정 (2026-07-29) — 이 래치는 Octave에 옮길 수 없다.**
>
> 위 인용의 마지막 줄이 왜 성립하는지를 보라: `cx.background_spawn`(`markdown.rs:973`).
> **Zed는 파싱을 백그라운드 스레드에서 한다.** 그래서 "이미 도는 중"이라는 상태가
> 존재하고, 래치가 의미를 갖는다.
>
> `react-markdown`은 **렌더 함수 안에서 동기 파싱**한다(`lib/index.js:175-179`:
> `post(processor.runSync(processor.parse(file), file), options)`). 한 스레드에서
> 동기 파싱 둘이 동시에 돌 수 없다 — **`pending`이 참인 순간이 존재하지 않는다.**
> 래치를 걸 대상이 없다.
>
> 이 문서 §5가 이미 올바른 답을 적어놨다: *"프레임 간 캐시 전무 … **완료 블록 memo가
> 낫다**"*. §8 항목 5가 그걸 무시하고 래치를 처방한 것이 오류다.
>
> **그런데 블록 memo도 채택하지 않았다.** 실측 결과 우리 파이프라인에서 13개 마크다운
> 구조 중 **4개가 어떤 경계 규칙으로도 안 고쳐진다**(참조형 링크·이미지, 각주, HTML
> 블록). 상세와 대안은 `rust-architecture-audit-2026-07.md` §2-1의 ⚠️ 블록에.
>
> **실제로 한 것**: 페이서의 커밋을 리빌과 분리해 속도 제한(`37d5f007a`).
> 20KB 답변이 메인스레드 73.6%(최악 261ms) → 13.5%(28ms). 조사한 제품 4곳
> (Streamdown, assistant-ui, LobeChat, open-webui) **전부가 커밋 속도 제한을 첫 번째
> 조치로** 하고 있었고, assistant-ui는 `minCommitMs`라는 같은 손잡이에 같은 이유를
> 적어놨다.
...
| `markdown.rs:2121` `MarkdownElement::id() -> None` | 프레임 간 캐시 전무, 매 프레임 `TextRun`/`StyledText` 재생성 | ~~완료 블록 memo가 낫다~~ → 실측 결과 **분할은 13개 중 4개 구조를 깬다**. 대신 커밋 속도 제한 (`37d5f007a`) |
...
**0~4는 완료됐다** (선행 감사의 ⚠️ 정정 블록에 각 항목의 근거가 있다).

| # | 할 일 | 출처 | 고치는 것 | 크기 | 상태 |
|---|---|---|---|---|---|
| **0** | `credential_helper_args` 잔해 삭제 | — | Rust 테스트 **18개** 부활 (20 아님) | 분 | ✅ `e073770d9` |
| **1** | **`derive_key`에** `OnceLock` 캐시 (**토큰은 금지**) | — | 감사 2-4: 시작마다 `ioreg` **18.4ms** 직렬화 | ~20줄 | ✅ `f1203f28a` |
| **2** | **`Drop`에서** `hard_kill` + `disarmed` (`restart` 아님 — 원안은 재시작 폭주) | **Zed §4.1** | 감사 3-5: CLI 손자 누수 (드랍 지점 **6곳**) | 소 | ✅ `9e0f346a3` |
| **3** | ~~릴레이 getter에 `?? bgTurnRunId`~~ → **불충분**, `lastRunId` + 호스트 절반 필요 → **9와 함께** | — | 감사 3-4 + 3-15(b) 거짓 보고 | 중 | ⏸ 보류 |
| **4** | `modelBodyBase`를 **threadId**로 키잉 (runId면 2턴째부터 CAS가 꺼짐) | — | 감사 3-1: 무음 유실 | 소 | ✅ `f7a47993c` |
| **4b** | 종료 중 재스폰 경쟁 (감사에 없던 항목) | — | 감사 3-15(a) | 소 | ✅ `5219d7239` |
| **5** | ~~단일 비행 파싱 래치~~ → **적용 불가** (§3-2 정정). 블록 분할도 대가가 더 큼 | ~~Zed §3-2~~ | — | ❌ 폐기 |
| **6** | 페이서의 **커밋**을 리빌과 분리해 속도 제한 (타이머·적응형 예산이 아니라) | assistant-ui `minCommitMs` / LobeChat | 감사 2-1: 초당 60회 전체 재파싱 | ✅ `37d5f007a` |
| **6b** | `PartList` 구독 좁히기 (감사 2-8이 "가상화 없음"으로 오분류) | — | Accept 한 번에 606ms 멈춤 | ✅ `fcc596b7a` |
...
| **13** | 쓰기를 "읽은 스냅샷 대비 diff"로. ~~chat도 `builtinTools`에서 Write/Edit 제거~~ (**이미 제거돼 있음** — §0.3 정정) | **Zed §3-3** | CAS 재시도 루프 제거(UX). 유실 뿌리는 3-1이었고 `f7a47993c`로 해결 | 대 |
...
> **⚠️ 정정 (2026-07-29)**: 13은 "에러가 전혀 없어야"의 핵심이 아니다. 그 자리는
> 감사 3-1(기준선 키잉)이었고 `f7a47993c`로 해결됐다. 13에 남은 것은 CAS 재시도
> 루프를 없애는 UX 개선과 `Bash` 구멍이다 — §0.3 정정 참조.
>
> 남은 "에러가 전혀 없어야" 항목은 **3(릴레이 라우팅) + 감사 3-15(b)
> `propose_*`의 거짓 보고**이고, 둘은 같은 뿌리라 **9와 함께** 가야 한다.
> 다만 `server.mjs:1913`이 *"Nothing ships a `background: true` agent today"*라고
> 적어둔 대로 지금은 잠복 상태다.

~~권장 진입점: **5번**(오늘 체감이 바뀜) 또는 **7번**(이후 절반의 전제).~~

> **2026-07-29**: 5번은 폐기, 6·6b가 그 자리를 대신해 완료됐다(스트리밍 20KB
> 73.6%→13.5%, Accept 한 번 606ms→0). **남은 권장 진입점은 7번** — 여전히
> 8·11·13의 전제다.
