...
> **재측정 (2026-07-30) — 위가 약속한 "6b 이후 재평가". 12번은 닫는다.**
> `src/chat/messages/transcript-bench.test.tsx`(진짜 `MessageRow` + 진짜
> `pendingChangesStore`, `BENCH=1`로만 실행):
>
> | 스레드 | 마운트 | 스토어 쓰기(Accept) | 키 입력(부모 리렌더) |
> |---|---|---|---|
> | 48개 × 0.6KB — **이 볼트의 실제 최악** | 118ms | 0.3ms | 0.3ms |
> | 50개 × 4KB | 333ms | 0.2ms | 0.2ms |
> | 50개 × 10KB | 761ms | 0.2ms | 0.3ms |
> | 200개 × 4KB | 1282ms | 0.2ms | 0.8ms |
>
> **Accept가 261ms → 0.2ms다.** 그리고 스레드 길이와 무관해졌다 — 200개에서도
> 0.2ms. 6b가 뿌리를 뽑은 게 맞다. 키 입력도 `React.memo`가 잡아준다.
>
> 그리고 위 표의 "50개 × 4KB" 자체가 **존재하지 않는 스레드다.** 이 볼트의
> 실제 스레드 127개를 세면 턴 수 중앙값 2 · p90 11 · **최대 48**이고, 턴 내용은
> 중앙값 30자 · p90 591자 · 최대 6.4KB다. 실제 최악(48 × 0.6KB)의 마운트는
> jsdom에서 118ms — 스레드를 열 때 한 번 든다.
>
> 그래서 **윈도잉 라이브러리는 넣지 않는다.** 남은 유일한 비용은 마운트이고,
> 그건 있지도 않은 크기에서만 문제가 된다. 벤치를 남겨 두었으니 그 전제가
> 바뀌면(긴 스레드가 실제로 생기면) 다시 재면 된다.
