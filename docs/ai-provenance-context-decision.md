...
> **상태: 보류 (2026-07-06)** — 에디터 blame UI(하이라이트/hover/offset 배선)·`git_blame_file`·
> 설정의 Version 뷰(사람용 히스토리 열람 창)를 모두 걷어냄. **auto-commit + 커밋 메시지(reason 포함)만
> 유지** — 출처/이유는 순수 AI-context로 두고 git에만 쌓인다(사람용 표면 없음). AI-context 회수
> (온디맨드 조회)는 회수 패턴을 더 조사한 뒤 구현한다. 이 문서는 그 결정의 근거 기록.
